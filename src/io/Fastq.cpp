 ...
 * @param cs is colorspace? (optional, default = false)
 */
Fastq::Fastq(const char *i, const char *s, bool cs) {
	id = string(i);
	sequence = string(s);
	colorspace = cs;
	have_quality = false;
 ...
 * @param cs is colorspace? (optional, default = false)
 */
Fastq::Fastq(const string &i, const string &s, bool cs) {
	id = string(i.c_str());
 ...
 * @param cs is colorspace? (optional, default = false)
 */
Fastq::Fastq(const char *i, const char *s, const char *c, bool cs) {
	id = string(i);
	sequence = string(s);
	comment = string(c);
	colorspace = cs;
 ...
 * @param cs is colorspace? (optional, default = false)
 */
Fastq::Fastq(const string &i, const string &s, const string &c, bool cs) {
	id = string(i.c_str());
	sequence = string(s.c_str());
 ...
 */
string Fastq::description(size_t n) const {
	stringstream s;
	s << "ID : " << id << endl;
	s << "STR: ";
...
istream & Fastq::read_from_fasta(istream & buffer) throw (Incorrect_Format) {
	have_quality = false;
	char b[BUFFER_LEN+1];
	b[BUFFER_LEN] = '\0';
	stringstream buf;
	...
	quality = string(sequence.size(),DEFAULT_QUALITY_CHAR);
	return buffer;
}

istream & Fastq::read_from_fastq(istream & buffer) throw (Incorrect_Format) {
	have_quality = true;
	char b[BUFFER_LEN+1];
	b[BUFFER_LEN] = '\0';
	char c = buffer.peek();
 ...
 * @param fasta the object where store the readed sequence
 */
istream & operator>> (istream & buffer, Fastq & fasta) throw (Incorrect_Format) {
	if (not buffer.eof()) {
		wchar_t c;
...
ostream & operator<< (ostream & buffer, const Fastq & fasta) {
	if (fasta.have_quality) {
		...
		buffer << '+' << endl;
		if (fasta.output_type == Fastq::illumina) {
			string temp;
			for (size_t i = 0; i < fasta.quality.size(); i++)
				temp.push_back(fasta.quality[i] + 31);
			buffer << temp << endl;
		} else
...
string Fastq::reverse_complement() const {
	return reverse_complement_standalone_str(sequence.c_str());
}

string Fastq::reverse_complement(size_t start, size_t stop) const throw (Data_Exception) {
 ...
 */
string Fastq::reverse() const {
	return reverse_standalone_str(sequence.c_str());
}

string Fastq::reverse(size_t start, size_t stop) const throw (Data_Exception) {
	if (start >=sequence.length())
		throw Data_Exception(0,sequence.length()-1,start,"Wrong start position");
	if (stop >= sequence.length())
		throw Data_Exception(0,sequence.length()-1,stop,"Wrong stop position");
...
t_quality Fastq::get_quality(size_t position) const throw (Data_Exception) {
...
Fastq::t_quality_vector Fastq::get_quality_vector() const {
	t_quality_vector v;
...
void Fastq::loweringNs() {
	for (size_t i = 0; i < sequence.size(); i++)
		if (sequence[i] == 'N')
			sequence[i] = 'n';
}

void Fastq::convertToColorspace() {
...
Fastq::FASTQ_type Fastq::check_FASTQ_type_file(const string &file) {
	Auto_Unzip is(file);
